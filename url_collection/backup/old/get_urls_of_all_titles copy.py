...
async def main(url):
    ...
    all_link_data_new = [] # remove duplicates, add videopage url
    ...
    for data in all_link_data:
        ...
        if "https://www.primevideo.com/detail/" in data['href']:
            if name in content_names:
                print("\n\n",name,"ALREADY FOUND!\n\n")
                continue
            #url_videopage = get_url_of_videopage(url_title)
            ...
            #print(name, url_title, "url_videopage:",url_videopage)
            #exit()
            #url_chunk = asyncio.get_event_loop().run_until_complete(get_url_of_vidchunk(url)) ######

            print("Text:", data['text'])
            print("Titlepage:", url_title)
...
count_dict = {}
for content, url in urls_main.items():
    print("*"*25,content,"\n",url,"*"*25,"\n\n\n\n")
    all_link_data_new = asyncio.get_event_loop().run_until_complete(main(url))
