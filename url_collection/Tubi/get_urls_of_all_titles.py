...
async def get_url_of_vidchunk(url):
    ...
    #url = "https://tubitv.com/movies/714158/nasty"
    ...
    def interception_fun(response):
        if (".mp4" in response.url and "fastly2.tubi.video" in response.url):
    ...
    try:
        ...
        await page.evaluate('''async () => {
            ...
            for (const button of buttons) {
                if (button.textContent.includes('Play') || button.textContent.includes('Watch')) {
                    button.click();
        ...
        while True:
            ...
            if elapsed_time > 40: # max timeout in sec
                break
...
async def scroll_to_bottom(page, increment=500):
    all_link_data = []
    shows_count = 0
    MAX_SHOWS = 10 ### CHANGE IT !!!!
    shows_url = []
    ...
    while True:
        ...
        for data in link_data:
            if "tubitv.com/series/" in data['href'] or "tubitv.com/movies/" in data['href']:
...
if __name__ == '__main__':
    ...
    for content, main_url in urls_main_top.items():
        ...
        for data in all_link_data_new:
            ...
            data['content'] = "tubi_" + content
            print("\n"*5)
            
            with open("url_tubi_" + content + ".csv", 'a', newline='') as csvfile:
